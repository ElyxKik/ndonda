...
class AuditFirebaseForm extends StatefulWidget {
  final String projectId;
  final String? documentId;
  final Map<String, dynamic>? data;

  const AuditFirebaseForm({
    super.key,
  ...
  @override
  State<AuditFirebaseForm> createState() => _AuditFirebaseFormState();
}

class _AuditFirebaseFormState extends State<AuditFirebaseForm> {
  ...
  void _loadExistingData() {
    ...
    if (data['dateSubmission'] != null) {
      try {
        if (data['dateSubmission'] is Timestamp) {
    ...
    if (data['environmentalIndicators'] != null) {
      final envData = data['environmentalIndicators'] as Map<String, dynamic>;
      envData.forEach((key, value) {
        if (_environmentalIndicators.containsKey(key)) {
          _environmentalIndicators[key] = value.toString();
        }
      });
    }

    if (data['socialIndicators'] != null) {
      final socialData = data['socialIndicators'] as Map<String, dynamic>;
      socialData.forEach((key, value) {
        if (_socialIndicators.containsKey(key)) {
          _socialIndicators[key] = value.toString();
        }
      });
    }

    if (data['quantitativeIndicators'] != null) {
      final quantData = data['quantitativeIndicators'] as Map<String, dynamic>;
      quantData.forEach((key, value) {
        if (_quantitativeIndicators.containsKey(key)) {
          _quantitativeIndicators[key]!.text = value.toString();
        }
      });
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (widget.documentId != null) {
        ...
        await _firestore.collection('audit').doc(widget.documentId).update(data);
      } else {
        data['createdAt'] = FieldValue.serverTimestamp();
        await _firestore.collection('audit').add(data);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.documentId != null ? 'Modifier Audit' : 'Nouvel Audit'),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(widget.documentId != null ? 'Mettre à jour' : 'Enregistrer'),
